...
(function (global, factory) {
  ...
  // 我们只对这7个方法做了拦截，如果用索引修改数组是无法实现响应式的
  var methodsToPatch = [
    'push',
    'pop',
    'shift',
  ...
  methodsToPatch.forEach(function (method) {
    ...
    def(arrayMethods, method, function mutator () {
      ...
      if (inserted) { ob.observeArray(inserted); } // 针对新插入的元素inserted
      // 通知更新
      // 每一个ob都有唯一的一个dep
      ob.dep.notify();
      return result
  ...
  function defineReactive (
    ...
    var dep = new Dep(); // 只要有一个属性，就有一个dep和它相对应
  ...
  function initState (vm) {
    vm._watchers = [];
    var opts = vm.$options;
    if (opts.props) { initProps(vm, opts.props); } // 属性初始化
    if (opts.methods) { initMethods(vm, opts.methods); } // 方法初始化
    // data处理：响应式处理
    if (opts.data) {
      initData(vm);
    } else {
  ...
  function initData (vm) {
    ...
    while (i--) {
      var key = keys[i];
      {
        if (methods && hasOwn(methods, key)) { // 判断重复，data里的属性是否在methods里存在
          warn(
      ...
      if (props && hasOwn(props, key)) { // 判断重复，data里的属性是否在props里存在
         warn(
  ...
  stateMixin(Vue); // 和组件状态相关：$data, $props, $set, $delete, $watch
  eventsMixin(Vue); // 和事件相关：$on, $emit, $once, $off
  lifecycleMixin(Vue); // 和生命周期相关：_update, $forceUpdate, $destroy
  renderMixin(Vue); // 和渲染相关：$nextTick, _render（_update, _render都是私有方法，不打算给用户使用的）
  ...
  Vue.prototype.$mount = function (
    ...
    // render不存在时才考虑el和template，template不存在时才考虑el。render > template > el
    if (!options.render) {
