...
export function createElement (
  ...
  // 真正创建虚拟节点 VNode 的函数：每个 VNode 有 children，children 每个元素也是一个 VNode，这样就形成了一个 VNode Tree
...
export function _createElement (
  ...
  */
  ...
  if (typeof tag === 'string') {
    ...
    // tag不是字符串的时候则是组件的构造类，比如：render: h => h(App)
    vnode = createComponent(tag, data, context, children)
  }
  if (Array.isArray(vnode)) {
    return vnode
  } else if (isDef(vnode)) {
    if (isDef(ns)) applyNS(vnode, ns) // 如果有名字空间，则递归所有子节点应用该名字空间
